...
def save_conv2caffe(weights, biases, conv_param):
    if biases is not None:
        conv_param[1].data[...] = biases.cpu().numpy()
    conv_param[0].data[...] = weights.cpu().numpy()
...
def save_fc2caffe(weights, biases, fc_param):
    ...
    fc_param[1].data[...] = biases.cpu().numpy()
    fc_param[0].data[...] = weights.cpu().numpy()


def save_bn2caffe(running_mean, running_var, bn_param):
    print(running_mean.cpu().numpy())
    print(running_var.cpu().numpy())
...
def save_scale2caffe(weights, biases, scale_param):
    scale_param[1].data[...] = biases.cpu().numpy()
    scale_param[0].data[...] = weights.cpu().numpy()
    ...
    def add_layer(func):
        ...
        if layer['type'] == 'Eltwise':
            if parent_type == 'MulBackward':
                ...
                raise Exception("Unsupported Layer type ", parent_type)   
            ...
            if parent_type == 'LeakyReLUBackward':
            ...
            if parent_type == 'UpsamplingNearest2d' or parent_type == 'UpsamplingNearest2dBackward':
            ...
            batch_norm_param['moving_average_fraction'] = func.momentum
            ...
            if parent_type == 'MaxPool2dBackward' or parent_type == 'MaxPool2DBackward':
        ...
        elif layer['type'] == 'pass':            
            try:
                parent_top = parent_bottoms[0]            
            except:
                parent_top = 'data'
        layer['top'] = parent_top  # reset layer['top'] as parent_top may change
        if layer['type'] != 'Reshape' and layer['type'] != 'pass':
        ...
        if var not in seen:
            ...
            if hasattr(var, 'saved_tensors'):
                for t in var.saved_tensors:
                    ...
                    add_nodes(t)
    if isinstance(top_var, list):
        add_nodes(top_var[0].grad_fn)
    else:
